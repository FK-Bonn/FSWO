...
\documentclass[%
final,%
%highlightlinks,% Uncomment to highlight links by using a different text color (default:orange)
...
\hypersetup{pdfinfo={%                  % Zusatzinformationen in PDF-Datei;
    ...
    CreationDate={D:20201109180700},    % Datum der Erstellung
    % um 19:07 (UTC+1) am 2020-11-09 wurde das Dokument natürlich nicht erstellt aber an dem Tag wurde der erste FK Beschluss beschlossen
                                        % (D:JJJJMMTThhmmss)
                                        % JJJJ  Jahr
                                        % MM    Monat
                                        % TT    Tag
                                        ...
                                        %
  ModDate={D:20210118180700},         % Datum der letzten Modifikation
...
\author{}
\date{{\itshape\normalsize%
  Beschlossen am 9.11.2020 und am 18.01.2021 durch die Fachschaftenkonferenz}}
\publishers{\itshape\normalsize%
  sowie am 18.01.2021 durch das Studierendenparlament.}
...
\newT{Alle Personen- und Funktionsbezeichnungen in dieser Ordnung gelten für alle Menschen unabhängig ihres Geschlechts in gleicher Weise.}
...
Fachschaften mit bis zu 500 Mitgliedern wählen nach \refClause{cls-wahl-fsr} dieser Ordnung in direkter Wahl einen Fachschaftsrat (FSR)\newT{, sofern die Fachschaftssatzung nicht die Wahl einer FSV vorsieht.}
Fachschaften mit 501 oder mehr Mitgliedern wählen nach \refClause{cls-wahl-fsv} dieser Ordnung in direkter Wahl eine Fachschaftsvertretung (FSV), die sodann einen FSR wählt (indirekte Wahl).
...
\change{\change{Aufgaben der Wahlleitung können nach Absprache auch von ihren einzelnen Mitgliedern wahrgenommen werden.}{%
...
Die Wahlleitung soll die ihr obliegenden Aufgaben selbst wahrnehmen.
...
\bemTi{Auf Wunsch des SGO-Ausschusses durch den Eigenaenderungsantrag der FK geändert.}
}
\end{contract}
...
Ein Abs. 6 wird eingeführt, der einen Verweis auf Einladung, Beschlussfähigkeit, Öffentlichkeit der Sitzungen und Protokollführung enthält –
...
Die Einladungen zu den Sitzungen des Wahlausschusses erfolgen durch öffentliche Bekanntmachung gemäß \refClause{cls-veroeffentlichungen-wa} dieser Ordnung durch die Wahlleitung spätestens 24 Stunden vor Sitzungsbeginn.
...
%Felix: Dies ist aber nur mit draft kaputt, mit final funktioniert alles...
...
Werbung für einzelne \change{Kandidaten}{Kandidierende} ist in Sicht- und Hörweite der Wahlurne und -kabine zu unterlassen und durch Wahlausschuss und Wahlhelfende zu unterbinden.
...
Die Wahlleitung benachrichtigt die gewählten Personen unverzüglich schriftlich oder per E- Mail von ihrer Wahl. %\label{cls-wahlergebnis:abs-gewaehlte-benachrichtigen}
...
\label{cls-wahlwiederholung-sonderfaelle:abs-keine-kandiaturen}

Wurde bei einer Wiederholungswahl nach Ablauf einer Nachfrist keine Kandidatur eingereicht, so enthält der Stimmzettel keine Kandidaturen, sondern lediglich das Freifeld gemäß \refClausePar{cls-stimmzettel:abs-freifeld}. \label{cls-wahlwiederholung-sonderfaelle:abs-wiederholgungswahl-keine-kandiaturen}
...
\refClausePar{cls-wahlwiederholung-sonderfaelle:abs-keine-kandiaturen} findet keine Anwendung.

Bis zum 25. Tag vor dem Wahltag legt der Wahlausschuss Ort und Zeit der Wahlvollversammlung fest.
Die Nummern \ref{cls-sitzungen-wa:beschluss-urnen}, \ref{cls-sitzungen-wa:beschluss-ende}  und \ref{cls-sitzungen-wa:beschluss-konsti} in \refClausePar{cls-sitzungen-wa:abs-wichtige-beschluesse} entfallen.
Nummer~\ref{cls-wahlbekanntmachung:wahltage} in \refClausePar{cls-wahlbekanntmachung:abs-inhalt} entfällt.
...
Die Wahl erfolgt unter TOP 4 gemäß \refClause{cls-wahlablauf}.
...
Das gewählte Gremium konstituiert sich abweichend von \refClause{cls-konstituierung} direkt nach Bekanntgabe des Wahlergebnisses unter TOP 6.
Abweichend von \refClausePar{cls-wahlergebnis:abs-wahlannahme} haben die Gewählten ab Bekanntgabe des Wahlergebnisses 7 Kalendertage Zeit, die Wahl anzunehmen.
...
\bemTi{Hier abgedruckt ist inzwischen der Kompromiss.}


\begin{contract}
\Clause{title={Option der Verhältniswahl}}
\label{cls-verhaeltniswahl}
Die Fachschaftssatzung kann festlegen, dass die Wahl abweichend von \refClausePar{cls-wahlgrundsaetze:abs-persoenlichkeitswahl} nicht als Persönlichkeitswahl, sondern als personalisierte Verhältniswahl durchgeführt wird. \label{cls-verhaeltniswahl:abs-satzung}
...
\makeatother
Neben dem in \refPar{cls-verhaeltniswahl:abs-satzung} genannten Fall wird die Wahl ebenfalls als
...
\item bei 19 zu wählenden Personen 10 Wahlberechtigte
\end{enumerate}
dies 25 Tage vor dem 1. Wahltag für diese Wahl verlangen. Das Verlangen ist der Wahlleitung einzureichen und von dieser zu prüfen. \label{cls-verhaeltniswahl:abs-verlangen}
\endgroup
...
Angekreuzt werden kann ein gesamter Wahlvorschlag oder eine einzelne Kandidatur. \label{cls-verhaeltniswahl:abs-stimmzettel}
...
Abweichend von der Regelung in \refClause{cls-wahlsystem} \refPar{cls-wahlsystem:abs-mindeststimme} und \refParN[arabic]{cls-wahlsystem:abs-nachruecken} ist keine Mindeststimmzahl erforderlich.

Die Bekanntgabe des Wahlergebnisses gemäß \refClause{cls-wahlergebnis} enthält zusätzlich die Zahl der auf jeden Wahlvorschlag entfallenden Stimmen und Sitze.

Wurde bei einer Wiederholungswahl nach Ablauf einer Nachfrist keine Kandidatur eingereicht (\refClausePar{cls-wahlwiederholung-sonderfaelle:abs-wiederholgungswahl-keine-kandiaturen}), so wird statt der Verhältniswahl eine Persönlichkeitswahl durchgeführt, bei der der Stimmzettel abweichend von \refPar{cls-verhaeltniswahl:abs-stimmzettel} lediglich das Freifeld gemäß \refClausePar{cls-stimmzettel:abs-freifeld} enthält.



Die übrigen Vorschriften dieser Wahlordnung sind entsprechend anzuwenden.

\change{Eine Festlegung nach Abs. 1 kann innerhalb der letzten 30 Tage vor Beginn einer Wahl nicht mehr mit Wirkung für diese Wahl hinzugefügt oder gestrichen werden.}{Eine Festlegung nach \refPar{cls-verhaeltniswahl:abs-satzung}, die nach Bestellung des Wahlausschusses  hinzugefügt oder gestrichen wird, entfaltet keine Wirkung mehr für diese Wahl.} %durch den Eigenaenderungsantrag der FK redaktionell korrigiert
...
\refPar{cls-verhaeltniswahl:abs-verlangen} bleibt unberührt.

Die Durchführung einer Verhältniswahl gemäß \refClause{cls-verhaeltniswahl} hat stets Vorrang vor der Durchführung einer Persönlichkeitswahl in einer Wahlvollversammlung gemäß \refClause{cls-wahlvollversammlung}.
\end{contract}
...
\vspace{2em}
{\itshape%
Ausgefertigt aufgrund der Beschlüsse der Fachschaftenkonferenz am 9.~November~2020 und am 18.~Januar~2021 sowie des Beschlusses des Studierendenparlaments am 18.~Januar~2021.

Bonn, der \today} %%%todo aendern
\vspace{1em}
\begin{center}
	Der Vorsitzende des  Fachschaftenkollektivs\\
 	Philipp Wippermann\\
 	\vspace{2em}
 	Der Erste Sprecher des Studierendenparlaments\\
 	Kay A.\ Frenken
\end{center}
